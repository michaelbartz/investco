...
from .models import (
    ...
    Stock, Bond, ETF, MutualFund, Retirement401k, Annuity,
...
def portfolio_performance(request, portfolio_id):
    ...
    # Aggregate historical values for all investments
    historical_data = defaultdict(lambda: Decimal('0'))
    for investment in investments:
        # Skip annuities - they don't use InvestmentValue
        if isinstance(investment, Annuity):
            # For annuities, just use current value (not historical tracking via InvestmentValue)
            continue

        values = investment.historical_values.filter(
            date__gte=start_date,
            date__lte=end_date
        ).order_by('date')

        for value in values:
            date_key = value.date.strftime('%Y-%m-%d')
            # Check if this is a share-based investment
            if hasattr(investment, 'shares'):
                historical_data[date_key] += value.price * investment.shares
            else:
                # For non-share investments (like real estate), price is total value
                historical_data[date_key] += value.price

    # Prepare chart data
    chart_dates = sorted(historical_data.keys())
    chart_values = [float(historical_data[date]) for date in chart_dates]
...
def investment_performance(request, investment_id):
    ...
    days_param = request.GET.get('days', '365' if isinstance(investment, Annuity) else '30')
    ...
    # Annuity-specific data: calculate total value over time from transactions
    if isinstance(investment, Annuity):
        ...
        # Get GWB (Guaranteed Withdrawal Balance) data from statements
        gwb_data = []
        ...
        if last_stmt_before_period and hasattr(last_stmt_before_period, 'annuitystatement'):
            if last_stmt_before_period.annuitystatement.remaining_guaranteed_balance:
                gwb_data.append({
                    'date': start_date.strftime('%Y-%m-%d'),
                    'value': float(last_stmt_before_period.annuitystatement.remaining_guaranteed_balance)
                })
        ...
        for stmt in statements:
            if hasattr(stmt, 'annuitystatement') and stmt.annuitystatement.remaining_guaranteed_balance:
                gwb_data.append({
                    'date': stmt.statement_date.strftime('%Y-%m-%d'),
                    'value': float(stmt.annuitystatement.remaining_guaranteed_balance)
                })
        ...
        # Get performance breakdown for additional metrics
...
def statement_detail(request, statement_id):
    ...
    if isinstance(statement, AnnuityStatement):
